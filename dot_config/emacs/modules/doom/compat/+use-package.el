;;; modules/doom/compat/+use-package.el -*- lexical-binding: t; -*-
;;; Commentary:
;;; Code:

(defvar +compat--deferred-packages-alist '(t))
        ...
        (use-package-concat
         `((fset ',fn
                 (lambda (&rest _)
                   ...
                   (when-let* ((deferral-list (assq ',name +compat--deferred-packages-alist)))
                     (dolist (hook (cdr deferral-list))
                       (advice-remove hook #',fn)
                       (remove-hook hook #',fn))
                     (cl-callf2 delq deferral-list +compat--deferred-packages-alist)
                     (unintern ',fn nil)))))
         (let (forms)
           (dolist (hook hooks forms)
             (push (if (string-match-p "-\\(?:functions\\|hook\\)$" (symbol-name hook))
                   ...
                   forms)))
         `((unless (assq ',name +compat--deferred-packages-alist)
             (push '(,name) +compat--deferred-packages-alist))
           (nconc (assq ',name +compat--deferred-packages-alist)
                  '(,@hooks)))
         (use-package-process-keywords name rest state))))))
