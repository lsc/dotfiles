...
You can install this fork of dirvish using [[https://github.com/radian-software/straight.el][straight.el]].
#+begin_src elisp
  (straight-use-package
   '(dirvish :type git :repo "latiagertrutis/dirvish" ))
  ;; Activate dirvish globally
  (with-eval-after-load 'dirvish
    (dirvish-override-dired-mode))
#+end_src


Or if configured to use ~use-package~:
#+begin_src elisp
  (use-package dirvish
    :straight (dirvish :type git :repo "latiagertrutis/dirvish")
    ;; Activate dirvish globally
    :init
    (dirvish-override-dired-mode))
#+end_src
...
To install it in Spacemacs, add the following under ~dotspacemacs-additional-packages~ inside ~.spacemacs~:
#+begin_src elisp
(dirvish :location (recipe :fetcher github :repo "latiagertrutis/dirvish" :files (:defaults "extensions/*.el")))  
#+end_src
